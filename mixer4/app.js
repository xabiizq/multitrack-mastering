(function () {
  ...
  function buildTone(ctx, input, settings) {
    ...
    n.treble = ctx.createBiquadFilter(); n.treble.type = 'highshelf'; n.treble.frequency.value = 8000; n.treble.gain.value = settings.treble;
  ...
  function addChannel(def) { var el = document.createElement('article'); el.id = 'channel-' + def.id; el.className = 'channel panel empty'; el.innerHTML = '<h2>'+def.name+'</h2><p class="track-name" id="name-'+def.id+'">Sin pista</p><label class="load-label">Cargar WAV<input id="file-'+def.id+'" type="file" accept=".wav,audio/wav"></label>'+meterHtml('track-'+def.id, 'CANAL')+'<div class="channel-controls">'+control('Volumen dB','tracks.'+def.id+'.volume',-24,6,.1)+control('Panorama','tracks.'+def.id+'.pan',-1,1,.01)+control('Graves','tracks.'+def.id+'.bass',-6,6,.1)+control('Medios','tracks.'+def.id+'.mid',-6,6,.1)+control('Agudos','tracks.'+def.id+'.treble',-6,6,.1)+control('Presencia / Ataque','tracks.'+def.id+'.presence',-1,1,.01)+'<div class="presence-scale"><span>SUAVE</span><span>NEUTRO</span><span>PRESENTE</span></div><div class="mini-buttons"><label class="switch"><input data-setting="tracks.'+def.id+'.mute" type="checkbox"> Mute</label><label class="switch"><input data-setting="tracks.'+def.id+'.solo" type="checkbox"> Solo</label></div>'+control('Envío Tape Echo','tracks.'+def.id+'.echoSend',0,1,.01)+control('Envío Dark Chamber','tracks.'+def.id+'.chamberSend',0,1,.01)+control('Saturación cinta','tracks.'+def.id+'.saturation',1,3,.01)+'</div>'; byId('channels').appendChild(el); byId('file-'+def.id).onchange=function(){if(this.files[0])loadFile(def,this.files[0]);}; setChannelEnabled(def.id, false); }
