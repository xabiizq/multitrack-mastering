(function () {
  ...
  var STORE_KEY = 'microphonMix60s4TrackSettings';
  ...
  var trackDefs = [
    { id: 'track1', name: 'PISTA 1' }, { id: 'track2', name: 'PISTA 2' },
    { id: 'track3', name: 'PISTA 3' }, { id: 'track4', name: 'PISTA 4' }
  ];
  var defaults = {
    tracks: {
      track1: { volume: 0, pan: 0, bass: 0, mid: 0, treble: 0, presence: 0, mute: false, solo: false, echoSend: 0, chamberSend: 0, saturation: 1.01 },
      track2: { volume: 0, pan: 0, bass: 0, mid: 0, treble: 0, presence: 0, mute: false, solo: false, echoSend: 0, chamberSend: 0, saturation: 1.01 },
      track3: { volume: 0, pan: 0, bass: 0, mid: 0, treble: 0, presence: 0, mute: false, solo: false, echoSend: 0, chamberSend: 0, saturation: 1.01 },
      track4: { volume: 0, pan: 0, bass: 0, mid: 0, treble: 0, presence: 0, mute: false, solo: false, echoSend: 0, chamberSend: 0, saturation: 1.01 }
    },
  ...
  function presenceValues(value) {
    var amount = Math.max(-1, Math.min(1, Number(value) || 0)), shaped = Math.pow(Math.abs(amount), 1.35), soft = amount < 0 ? shaped : 0, forward = amount > 0 ? shaped : 0;
    return { eq: forward * 8 - soft * 5.8, attack: forward * 0.48, threshold: -12 - soft * 18, ratio: 1 + soft * 2, makeup: dbToGain(-forward * 1.7 + soft * 0.6) };
  }

  function buildTone(ctx, input, settings) {
    var n = {};
    n.bass = ctx.createBiquadFilter(); n.bass.type = 'lowshelf'; n.bass.frequency.value = 100; n.bass.gain.value = settings.bass;
    n.mid = ctx.createBiquadFilter(); n.mid.type = 'peaking'; n.mid.frequency.value = 1000; n.mid.Q.value = 0.5; n.mid.gain.value = settings.mid;
    n.treble = ctx.createBiquadFilter(); n.treble.type = 'highshelf'; n.treble.frequency.value = 4000; n.treble.gain.value = settings.treble;
    input.connect(n.bass); n.bass.connect(n.mid); n.mid.connect(n.treble); n.output = n.treble;
    return n;
  }
  ...
  function buildGraph(ctx, when, pos, options) {
    ...
    trackDefs.forEach(function (t) {
      ...
      n.source.connect(n.saturation); n.tone = buildTone(ctx, n.saturation, s); n.presence = buildPresence(ctx, n.tone.output, s.presence); n.presence.output.connect(n.volume); n.volume.connect(n.pan); n.pan.connect(g.mixIn); n.pan.connect(n.echoSend); n.echoSend.connect(g.echoDelay); n.pan.connect(n.chamberSend); n.chamberSend.connect(g.chamberDelay);
  ...
  function updateAudioParameter(path) {
    ...
    if (p[0] === 'tracks') { t = graph.tracks[p[1]]; s = settings.tracks[p[1]]; if (!t) return; if (p[2] === 'volume' || p[2] === 'mute' || p[2] === 'solo') updateTrackGains(); else if (p[2] === 'pan') smooth(t.pan.pan, s.pan, audioContext); else if (p[2] === 'bass' || p[2] === 'mid' || p[2] === 'treble') smooth(t.tone[p[2]].gain, s[p[2]], audioContext); else if (p[2] === 'presence') updatePresenceNodes(t.presence, s.presence, audioContext); else if (p[2] === 'echoSend') smooth(t.echoSend.gain, s.echoSend, audioContext); else if (p[2] === 'chamberSend') smooth(t.chamberSend.gain, s.chamberSend, audioContext); else if (p[2] === 'saturation') t.saturation.curve = makeCurve(s.saturation); }
  ...
  function addChannel(def) { var el = document.createElement('article'); el.id = 'channel-' + def.id; el.className = 'channel panel empty'; el.innerHTML = '<h2>'+def.name+'</h2><p class="track-name" id="name-'+def.id+'">Sin pista</p><label class="load-label">Cargar WAV<input id="file-'+def.id+'" type="file" accept=".wav,audio/wav"></label>'+meterHtml('track-'+def.id, 'CANAL')+'<div class="channel-controls">'+control('Volumen dB','tracks.'+def.id+'.volume',-24,6,.1)+control('Panorama','tracks.'+def.id+'.pan',-1,1,.01)+control('Graves','tracks.'+def.id+'.bass',-12,12,.1)+control('Medios','tracks.'+def.id+'.mid',-12,12,.1)+control('Agudos','tracks.'+def.id+'.treble',-12,12,.1)+control('Presencia / Ataque','tracks.'+def.id+'.presence',-1,1,.01)+'<div class="presence-scale"><span>SUAVE</span><span>NEUTRO</span><span>PRESENTE</span></div><div class="mini-buttons"><label class="switch"><input data-setting="tracks.'+def.id+'.mute" type="checkbox"> Mute</label><label class="switch"><input data-setting="tracks.'+def.id+'.solo" type="checkbox"> Solo</label></div>'+control('Envío Tape Echo','tracks.'+def.id+'.echoSend',0,1,.01)+control('Envío Dark Chamber','tracks.'+def.id+'.chamberSend',0,1,.01)+control('Saturación cinta','tracks.'+def.id+'.saturation',1,3,.01)+'</div>'; byId('channels').appendChild(el); byId('file-'+def.id).onchange=function(){if(this.files[0])loadFile(def,this.files[0]);}; setChannelEnabled(def.id, false); }
  ...
  function loadFile(def,file){if(playing)stop();byId('loadStatus').textContent='Cargando '+file.name+'…';var reader=new FileReader();reader.onerror=function(){byId('loadStatus').textContent='No se pudo leer el WAV.';};reader.onload=function(){audioContext.decodeAudioData(reader.result,function(buffer){buffers[def.id]=buffer;byId('name-'+def.id).textContent=file.name;setChannelEnabled(def.id,true);refreshControls();resetMeters();updateAvailability();},function(){byId('loadStatus').textContent='WAV no compatible: '+file.name;});};reader.readAsArrayBuffer(file);}
