(function () {
  ...
  function panNode(ctx, pan, channelCount) {
    var input = ctx.createGain(), mono = input, splitter, sumL, sumR;
    var left = ctx.createGain(), right = ctx.createGain(), merger = ctx.createChannelMerger(2);

    /* Fold a stereo RAW file to mono before panning. Pan therefore positions the
       complete track instead of behaving like a stereo balance control. */
    if (channelCount > 1) {
      splitter = ctx.createChannelSplitter(2); sumL = ctx.createGain(); sumR = ctx.createGain(); mono = ctx.createGain();
      sumL.gain.value = 0.5; sumR.gain.value = 0.5;
      input.connect(splitter); splitter.connect(sumL, 0); splitter.connect(sumR, 1); sumL.connect(mono); sumR.connect(mono);
    }

    /* Equal-power law: centre is -3.01 dB in each output and total power stays
       constant over the full travel. There is no global pan stage. */
    left.gain.value = Math.cos((pan + 1) * Math.PI / 4);
    right.gain.value = Math.sin((pan + 1) * Math.PI / 4);
    mono.connect(left); mono.connect(right); left.connect(merger, 0, 0); right.connect(merger, 0, 1);
    return { input: input, output: merger };
  }
  ...
  function buildGraph(ctx, when, pos) {
    ...
    trackDefs.forEach(function (t) {
      var s = settings.tracks[t.id], src = ctx.createBufferSource(), sat = ctx.createWaveShaper(), punch = ctx.createBiquadFilter(), gain = ctx.createGain(), pan = panNode(ctx, s.pan, buffers[t.id].numberOfChannels), echoSend = ctx.createGain(), chSend = ctx.createGain();
